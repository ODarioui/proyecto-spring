...
public class AbsenseServiceImpl implements AbsenceService {
    ...
    AbsenseServiceImpl(EmployeeRepository employeeRepository, AbsenceRepository absenceRepository,
    ...
    public Absence registerAbsence(AbsenceRequestDto newAbsence) {
        ...
        if (!employee.isPresent()) {
            throw new IllegalArgumentException(
                    "No existe el empleado con el id : " + newAbsence.getEmployeeId());
        }
    ...
    public void updateStatus(Long absenceId, String newStatus) {
        ...
        if (!absence.isPresent())
            throw new IllegalArgumentException("No existe la ausencia con el id: " + absenceId);
        ...
        if ("Aprobada".equalsIgnoreCase(newStatus)) {
            ...
            notificationService.printNotification(
                    employee.getUser().getEmail(),
                    "Ausencia aprobada",
                    "Tu ausencia del " + absence.get().getStartDate() + " al " + absence.get().getEndDate()
                            + " ha sido aprobada. Total días de ausencia: " + totalDays);
